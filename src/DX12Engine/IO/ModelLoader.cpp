...
namespace DX12Engine
{
	...
	static float ReadScalarFloat(const uint8_t* p, int componentType, bool normalized)
	{
		...
		case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:
			return normalized ? *p / 255.0f : static_cast<float>(*p);
		case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
			return normalized ? *reinterpret_cast<const uint16_t*>(p) / 65535.0f
	...
	static bool ParseCookedLodManifest(const std::string& jsonText, std::vector<CookedPrimitiveLodRecord>& outRecords)
	{
		...
		try
		{
			...
			for (const nlohmann::json& primJson : root["primitives"])
			{
				...
				if (primRecord.meshIndex < 0 || primRecord.primitiveIndex < 0)
					continue;
									...
									int parentAssetIdx)
	{
		...
		if (gltfNode.matrix.size() == 16)
		{
			...
			DirectX::XMMATRIX m = DirectX::XMMatrixSet(
				static_cast<float>(gltfNode.matrix[0]), static_cast<float>(gltfNode.matrix[4]), static_cast<float>(gltfNode.matrix[8]), static_cast<float>(gltfNode.matrix[12]), static_cast<float>(gltfNode.matrix[1]), static_cast<float>(gltfNode.matrix[5]), static_cast<float>(gltfNode.matrix[9]), static_cast<float>(gltfNode.matrix[13]), static_cast<float>(gltfNode.matrix[2]), static_cast<float>(gltfNode.matrix[6]), static_cast<float>(gltfNode.matrix[10]), static_cast<float>(gltfNode.matrix[14]), static_cast<float>(gltfNode.matrix[3]), static_cast<float>(gltfNode.matrix[7]), static_cast<float>(gltfNode.matrix[11]), static_cast<float>(gltfNode.matrix[15]));
	...
	static bool MapCookedModelBinary(const fs::path& modelBinaryPath, CookedModelMappedView& outMapped, std::string& outError)
	{
		...
		if (!GetChunkViewByType(outMapped, CookedModelChunkType::Vertices, outMapped.VerticesChunk, outError))
			return false;
		if (!GetChunkViewByType(outMapped, CookedModelChunkType::IndicesLOD, outMapped.IndicesChunk, outError))
			return false;
		if (!GetChunkViewByType(outMapped, CookedModelChunkType::Meshes, outMapped.MeshesChunk, outError))
			return false;
		if (!GetChunkViewByType(outMapped, CookedModelChunkType::Nodes, outMapped.NodesChunk, outError))
			return false;
		if (!GetChunkViewByType(outMapped, CookedModelChunkType::Materials, outMapped.MaterialsChunk, outError))
			return false;
		if (!GetChunkViewByType(outMapped, CookedModelChunkType::Strings, outMapped.StringsChunk, outError))
			return false;

		if (!MapChunkStruct(outMapped.VerticesChunk, 0, outMapped.VertexHeader, outError, "vertices"))
			return false;
		if (!MapChunkArray(outMapped.VerticesChunk, outMapped.VertexHeader->PrimitiveTableOffset, outMapped.VertexHeader->PrimitiveCount, outMapped.VertexPrimitiveRecords, outError, "vertices table"))
			return false;
		if (!MapChunkArray(outMapped.VerticesChunk, outMapped.VertexHeader->VertexDataOffset, outMapped.VertexHeader->VertexCount, outMapped.Vertices, outError, "vertex stream"))
			return false;

		if (!MapChunkStruct(outMapped.IndicesChunk, 0, outMapped.IndexHeader, outError, "indices_lod"))
			return false;
		if (!MapChunkArray(outMapped.IndicesChunk, outMapped.IndexHeader->LodRecordOffset, outMapped.IndexHeader->LodRecordCount, outMapped.IndexLodRecords, outError, "lod table"))
			return false;
		if (!MapChunkArray(outMapped.IndicesChunk, outMapped.IndexHeader->IndexDataOffset, outMapped.IndexHeader->TotalIndexCount, outMapped.IndexData, outError, "index stream"))
			return false;

		if (!MapChunkStruct(outMapped.MeshesChunk, 0, outMapped.MeshesHeader, outError, "meshes"))
			return false;
		if (!MapChunkArray(outMapped.MeshesChunk, outMapped.MeshesHeader->MeshTableOffset, outMapped.MeshesHeader->MeshCount, outMapped.MeshRecords, outError, "mesh table"))
			return false;
		if (!MapChunkArray(outMapped.MeshesChunk, outMapped.MeshesHeader->PrimitiveTableOffset, outMapped.MeshesHeader->PrimitiveCount, outMapped.PrimitiveRecords, outError, "primitive table"))
			return false;

		if (!MapChunkArray(outMapped.NodesChunk, 0, outMapped.NodesChunk.Desc->ElementCount, outMapped.NodeRecords, outError, "node table"))
			return false;

		if (!MapChunkStruct(outMapped.MaterialsChunk, 0, outMapped.MaterialsHeader, outError, "materials"))
			return false;
		if (!MapChunkArray(outMapped.MaterialsChunk, outMapped.MaterialsHeader->MaterialTableOffset, outMapped.MaterialsHeader->MaterialCount, outMapped.MaterialRecords, outError, "material table"))
			return false;
		if (!MapChunkArray(outMapped.MaterialsChunk, outMapped.MaterialsHeader->TextureRefTableOffset, outMapped.MaterialsHeader->TextureRefCount, outMapped.TextureRefRecords, outError, "texture ref table"))
			return false;

		if (!TryGetChunkViewByType(outMapped, CookedModelChunkType::Animations, outMapped.AnimationsChunk, outError))
			return false;
		if (outMapped.AnimationsChunk.Data && outMapped.AnimationsChunk.Size > 0)
		{
			if (!MapChunkStruct(outMapped.AnimationsChunk, 0, outMapped.AnimationsHeader, outError, "animations"))
				return false;
			if (!MapChunkArray(outMapped.AnimationsChunk, outMapped.AnimationsHeader->ClipTableOffset, outMapped.AnimationsHeader->ClipCount, outMapped.AnimationClipRecords, outError, "animation clip table"))
				return false;
			if (!MapChunkArray(outMapped.AnimationsChunk, outMapped.AnimationsHeader->SamplerTableOffset, outMapped.AnimationsHeader->SamplerCount, outMapped.AnimationSamplerRecords, outError, "animation sampler table"))
				return false;
			if (!MapChunkArray(outMapped.AnimationsChunk, outMapped.AnimationsHeader->ChannelTableOffset, outMapped.AnimationsHeader->ChannelCount, outMapped.AnimationChannelRecords, outError, "animation channel table"))
				return false;
			if (!MapChunkArray(outMapped.AnimationsChunk, outMapped.AnimationsHeader->TimeDataOffset, outMapped.AnimationsHeader->TimeKeyCount, outMapped.AnimationTimes, outError, "animation time keys"))
				return false;
			if (!MapChunkArray(outMapped.AnimationsChunk, outMapped.AnimationsHeader->TranslationDataOffset, outMapped.AnimationsHeader->TranslationKeyCount, outMapped.AnimationTranslations, outError, "animation translation keys"))
				return false;
			if (!MapChunkArray(outMapped.AnimationsChunk, outMapped.AnimationsHeader->RotationDataOffset, outMapped.AnimationsHeader->RotationKeyCount, outMapped.AnimationRotations, outError, "animation rotation keys"))
				return false;
			if (!MapChunkArray(outMapped.AnimationsChunk, outMapped.AnimationsHeader->ScaleDataOffset, outMapped.AnimationsHeader->ScaleKeyCount, outMapped.AnimationScales, outError, "animation scale keys"))
				return false;
		}
		...
		std::string& outError)
	{
		...
		for (std::uint32_t materialIndex = 0; materialIndex < mapped.MaterialsHeader->MaterialCount; ++materialIndex)
		{
			...
			pbrMaterial->SetAlbedo({ materialRecord.BaseColorFactor[0],
			...
			pbrMaterial->SetEmissive({ materialRecord.EmissiveFactor[0],
	...
	void ModelLoader::TryApplyCookedMeshLods(const std::string& modelName, ModelAsset& modelAsset)
	{
		...
		for (const CookedPrimitiveLodRecord& primRecord : lodRecords)
		{
			...
			if (!primitive || !primitive->HasGeometry())
				continue;
			...
			{
				if (lodRecord.level <= 0 || lodRecord.output.empty())
					continue;
