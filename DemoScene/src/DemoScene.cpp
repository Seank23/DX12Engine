...
namespace DX12EngineDemo
{
	...
	void DemoScene::Init()
	{
		...
		DX12Engine::ModelNode cubeRoot;
		cubeRoot.Name = "Root";
		cubeRoot.ParentIndex = -1;
		cubeRoot.MeshIndex = -1;
		size_t cubeRootIdx = cubeModel->AddNode(cubeRoot);
		DX12Engine::ModelNode cubeNode;
		cubeNode.Name = "Cube";
		cubeNode.ParentIndex = (int)cubeRootIdx;
		cubeNode.MeshIndex = cubeMeshIdx;
		...
		DX12Engine::ModelNode sphereRoot;
		sphereRoot.Name = "Root";
		sphereRoot.ParentIndex = -1;
		sphereRoot.MeshIndex = -1;
		size_t sphereRootIdx = sphereModel->AddNode(sphereRoot);
		DX12Engine::ModelNode sphereNode;
		sphereNode.Name = "Sphere";
		sphereNode.ParentIndex = (int)sphereRootIdx;
		sphereNode.MeshIndex = sphereMeshIdx;
		...
		DX12Engine::ModelNode floorRoot;
		floorRoot.Name = "Root";
		floorRoot.ParentIndex = -1;
		floorRoot.MeshIndex = -1;
		size_t floorRootIdx = floorModel->AddNode(floorRoot);
		DX12Engine::ModelNode floorNode;
		floorNode.Name = "Floor";
		floorNode.ParentIndex = (int)floorRootIdx;
		floorNode.MeshIndex = floorMeshIdx;
